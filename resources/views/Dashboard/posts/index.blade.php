<x-layout title="Content Management">
    <div class="pt-8 pb-section-gap px-gutter max-w-container-max mx-auto">
        <header class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div class="max-w-2xl">
                <h1 class="font-display-lg-mobile text-[22px] font-bold text-on-surface truncate">Content Management</h1>
                <p class="font-body-md text-on-surface-variant">Manage your posts, track engagement, and control
        ...
        <div class="flex gap-1 border-b border-outline-variant mb-6 overflow-x-auto">
            @foreach ($status_options as $option)
                ...
                <a href="{{ route('dashboard.posts.index', ['status' => strtolower($option['name'])]) }}"
                    ...
                    {{ $option['name'] }}
                    <span
                        class="text-metadata {{ $isActive ? 'text-primary' : 'text-outline' }}">({{ $option['count'] }})</span>
                </a>
            @endforeach
        </div>
        ...
        <div class="space-y-3">
            @foreach ($posts as $post)
                ...
                @endphp
                <article
                    class="bg-surface-container-lowest p-4 sm:p-5 rounded-xl border border-outline-variant hover:border-primary/30 transition-colors">
                    <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-4 lg:gap-6 shrink-0">
                            ...
                            @if ($post->trashed())
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border bg-red-50 text-red-700 border-red-200">
                            ...
                            <div class="flex gap-1">
                                @if ($post->trashed())
                                    ...
                                    <form class="hidden" id="restorepost{{ $post->id }}"
