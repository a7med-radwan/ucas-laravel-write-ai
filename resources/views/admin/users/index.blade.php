<x-layout title="Manage Users">
    <div class="pt-8 pb-section-gap px-gutter max-w-container-max mx-auto">
        <header class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div class="max-w-2xl">
                <h1 class="font-display-lg-mobile text-[22px] font-bold text-on-surface truncate">User Management</h1>
                <p class="font-body-md text-on-surface-variant">Manage platform users, assign roles, and control account
                    access levels.</p>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-surface-container-low border-b border-outline-variant">
                        <th
                            class="px-5 py-3.5 font-ui-label text-ui-label font-bold text-on-surface uppercase tracking-wider">
                            User</th>
                        <th
                            class="px-5 py-3.5 font-ui-label text-ui-label font-bold text-on-surface uppercase tracking-wider">
                            Email</th>
                        ...
                        <th
                            ...
                            Joined</th>
                        <th
                            class="px-5 py-3.5 font-ui-label text-ui-label font-bold text-on-surface uppercase tracking-wider text-right">
                            Actions</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-surface/60 transition-colors">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-full bg-primary-fixed flex items-center justify-center text-on-primary-fixed font-bold text-sm overflow-hidden shrink-0">
                                        @if ($user->avatar)
                                    ...
                                    <div class="min-w-0">
                                        <div class="font-ui-label text-ui-label font-bold text-on-surface truncate">
                                            {{ $user->name }}</div>
                                        <div class="font-metadata text-metadata text-on-surface-variant truncate">
                                            {{ '@' . $user->username }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4 font-ui-label text-ui-label text-on-surface-variant">{{ $user->email }}
                            </td>
                            <td class="px-5 py-4">
                                <span
                                    class="inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $typeBadge }}">
                                    {{ ucfirst($user->type) }}
                                </span>
                            </td>
                            <td class="px-5 py-4">
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $statusBadge[0] }}">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @forelse ($user->roles as $role)
                                        <span
                                            class="px-2 py-0.5 bg-primary-fixed text-on-primary-fixed rounded text-[11px] font-bold">{{ $role->name }}</span>
                                    @empty
                            ...
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-1">
                                    ...
                                    @can('delete', $user)
                                        ...
                                        <form class="hidden" id="deleteuser{{ $user->id }}"
